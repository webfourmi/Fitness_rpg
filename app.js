...
const coaches = {
  korvan: {
    ...
    image: "assets/coach/korvan/idle.png",
    fallbackImage: "assets/coach/korvan/image.jpg",
    poses: {
      idle: "assets/coach/korvan/idle.png",
      welcome: "assets/coach/korvan/welcome.png",
      explain: "assets/coach/korvan/explain.png",
      motivate: "assets/coach/korvan/motivate.png",
      warmup: "assets/coach/korvan/warmup.png",
      walk: "assets/coach/korvan/walk.png",
      bike: "assets/coach/korvan/bike.png",
      squats: "assets/coach/korvan/squats.png",
      core: "assets/coach/korvan/core.png",
      stretch: "assets/coach/korvan/stretch.png",
      victory: "assets/coach/korvan/victory.png",
      levelup: "assets/coach/korvan/levelup.png",
    },
    start: [
...
const el = {
  ...
  heroCreationFields: $("#heroCreationFields"),
  homeHeroSummary: $("#homeHeroSummary"),
  homeCoachSummary: $("#homeCoachSummary"),
  setupTitle: $("#setupTitle"),
  setupHelp: $("#setupHelp"),
  appVersionLabel: $("#appVersionLabel"),
  appVersionLabelEditor: $("#appVersionLabelEditor"),
  name: $("#playerNameInput"),
  avatar: $("#avatarSelect"),
  create: $("#createProfileBtn"),
  cont: $("#continueProfileBtn"),
  changeCoach: $("#changeCoachBtn"),
  newHero: $("#startCreateHeroBtn"),
  saveCoach: $("#saveCoachBtn"),
  cancel: $("#cancelSetupBtn"),
  ...
  coachName: $("#coachName"),
  coachPortrait: $("#coachPortrait"),
  coachMsg: $("#coachMessage"),
  newMsg: $("#newCoachMessageBtn"),
  choice: $("#coachChoiceGrid"),
  sportHub: $("#sportHub"),
  musicPage: $("#musicPage"),
  questsPage: $("#questsPage"),
  weekPage: $("#weekPage"),
  badgesPage: $("#badgesPage"),
  musicSummary: $("#currentMusicSummary"),
  questSummary: $("#currentQuestSummary"),
  openMusic: $("#openMusicBtn"),
  openQuests: $("#openQuestsBtn"),
  openWeek: $("#openWeekBtn"),
  openBadges: $("#openBadgesBtn"),
  questsList: $("#questsList"),
  resetDaily: $("#resetDailyBtn"),
  weekList: $("#weekList"),
...
function levelInfo(xp) {
  ...
  return { level, currentXp: remaining, nextXp: need(level) };
}
...
function rank(stageNumber) {
  return ["", "Débutant", "Mise en forme", "Sportif régulier", "Athlétique", "Héros confirmé"][stageNumber];
}

function coachImg(id, wantedPose = "idle") {
  const coach = coaches[id];
  return coach?.poses?.[wantedPose] || coach?.fallbackImage || coach?.image || "";
}

function safeCoachImage(img, id, wantedPose = "idle") {
  const coach = coaches[id];
  const preferred = coachImg(id, wantedPose);
  const fallback = coach?.fallbackImage || coach?.image || preferred;
  img.onerror = () => {
    img.onerror = null;
...
function selectedCoach() {
  return $('input[name="coach"]:checked')?.value || "korvan";
}

function msg(type) {
  const coach = coaches[profile.coach];
  return pick(coach[type] || coach.start);
}

function questMsg(questId) {
  const coach = coaches[profile.coach];
  const list = coach.byQuest?.[questId];
  return list ? pick(list) : msg("complete");
}

function log(text) {
  profile.log.unshift(`${new Date().toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })} · ${text}`);
  profile.log = profile.log.slice(0, LOG_LIMIT);
}

function doneToday() {
  return profile.completedByDate[today()] || [];
}

function setDone(list) {
  profile.completedByDate[today()] = list;
}
...
function updateStreak() {
  ...
  const ky = `${y.getFullYear()}-${String(y.getMonth() + 1).padStart(2, "0")}-${String(y.getDate()).padStart(2, "0")}`;
  profile.streak = profile.lastActiveDate === ky ? profile.streak + 1 : 1;
  profile.lastActiveDate = t;
}
...
function complete(id) {
  ...
  profile.xp += quest.xp;
  pose = questPoseMap[id] || "victory";
  log(`+${quest.xp} XP · ${quest.title}`);
  ...
  if (newLevel > oldLevel) {
    ...
    el.coachMsg.textContent = questMsg(id);
  }
...
function render() {
  ...
  const isDashboard = hasProfile && ["dashboard", "music", "quests", "week", "badges"].includes(currentView);
  ...
  if (!isDashboard) {
    ...
    const isCoach = hasProfile && currentView === "coach";
    ...
    if (isHome) {
      ...
      el.homeHeroSummary.textContent = `${profile.name} · ${avatarLabels[profile.avatar]} · Niveau ${info.level} · ${profile.totalXp} XP`;
      el.homeCoachSummary.textContent = `Coach actuel : ${coaches[profile.coach].fullName}`;
      return;
    }

    el.heroCreationFields.classList.toggle("hidden", isCoach);
    el.create.classList.toggle("hidden", isCoach);
    el.saveCoach.classList.toggle("hidden", !isCoach);
    el.cancel.classList.toggle("hidden", !hasProfile);
    ...
    if (isCoach) selectCoach(profile.coach);
    syncCards();
    return;
  }
  ...
  if (currentView === "music") pose = "motivate";
  if (currentView === "quests") pose = "explain";
  if (currentView === "week") pose = "welcome";
  if (currentView === "badges") pose = "victory";

  const info = levelInfo(profile.totalXp);
  const currentStage = stage(info.level);
  const completed = doneToday();
  const coach = coaches[profile.coach];

  el.heroName.textContent = profile.name;
  el.avatarLabel.textContent = avatarLabels[profile.avatar];
  el.rank.textContent = `${rank(currentStage)} · stade ${currentStage}`;
  el.level.textContent = `Niveau ${info.level}`;
  el.xp.textContent = `${info.currentXp} / ${info.nextXp} XP`;
  el.bar.style.width = `${Math.min(100, Math.round((info.currentXp / info.nextXp) * 100))}%`;
  el.total.textContent = profile.totalXp;
  el.streak.textContent = profile.streak;
  el.done.textContent = completed.length;
  el.coachName.textContent = coach.fullName;
  safeCoachImage(el.coachPortrait, profile.coach, pose);
  el.coachPortrait.alt = coach.fullName;
  el.heroPortrait.innerHTML = heroSvg(currentStage);
...
function renderQuests(completed) {
  ...
  quests.forEach((quest) => {
    ...
    item.innerHTML = `<div class="quest-title-row"><strong>${quest.title}</strong><span class="quest-xp">+${quest.xp} XP</span></div><p class="quest-meta">${quest.description}</p><p class="quest-meta">Stat : ${quest.stat}</p><button class="${isDone ? "ghost-btn" : "primary-btn"}" ${isDone ? "disabled" : ""}>${isDone ? "Déjà accomplie" : "Valider la quête"}</button>`;
    item.querySelector("button").onclick = () => complete(quest.id);
...
function renderWeek() {
  ...
  for (let i = 0; i < 7; i += 1) {
    ...
    item.className = `week-day${key === today() ? " today" : ""}${count ? " active" : ""}`;
    item.innerHTML = `<strong>${names[i]}</strong><span>${count} quête${count > 1 ? "s" : ""}</span>`;
...
function count(id) {
  return allDone().filter((questId) => questId === id).length;
}

function renderBadges() {
  if (!el.badgesList || !profile) return;
  const total = allDone().length;
  const full = Object.values(profile.completedByDate || {}).some((completed) => quests.every((quest) => completed.includes(quest.id)));
  const badges = [
    ["👣", "Premier pas", "Valider une première quête.", total >= 1],
    ["🔥", "Élan du héros", "Valider 3 quêtes au total.", total >= 3],
    ["🏆", "Journée complète", "Valider toutes les quêtes d’une journée.", full],
    ["📅", "Régulier", "Atteindre 3 jours de série.", profile.streak >= 3],
    ["🚴", "Cycliste novice", "Valider 5 séances de vélo.", count("bike") + count("bike15") >= 5],
    ["🧘", "Pilates novice", "Valider 3 séances de Pilates.", count("pilates10") + count("pilates30") + count("pilates45") >= 3],
    ["💪", "Force tranquille", "Valider 10 quêtes de force, gainage ou Pilates.", count("squats") + count("core") + count("pilates10") + count("pilates30") + count("pilates45") >= 10],
  ];

  el.badgesList.innerHTML = "";
  badges.forEach((badge) => {
    const item = document.createElement("article");
    item.className = `badge-item${badge[3] ? " unlocked" : ""}`;
    item.innerHTML = `<div class="badge-icon">${badge[0]}</div><div><strong>${badge[1]}</strong><p>${badge[2]}</p><span>${badge[3] ? "Débloqué" : "Verrouillé"}</span></div>`;
...
function summaries() {
  const completed = doneToday();
  const lastQuest = quests.find((quest) => quest.id === completed.at(-1));
  el.questSummary.textContent = lastQuest ? `Dernière quête : ${lastQuest.title} · ${lastQuest.description}` : "Aucune quête validée aujourd’hui.";
  el.musicSummary.textContent = pulseOn ? "Pulse épique en cours" : el.audio?.src ? "Musique locale chargée" : "Aucune musique choisie.";
}

function syncCards() {
  $$(".coach-choice-card").forEach((card) => {
    const id = card.dataset.coachCard;
    const input = card.querySelector('input[name="coach"]');
    card.classList.toggle("active", input.checked);
    const img = card.querySelector("img");
    if (img && coaches[id]) {
      safeCoachImage(img, id, "idle");
      img.alt = coaches[id].fullName;
    }
  });
}

function selectCoach(id) {
  const input = $(`input[name="coach"][value="${id}"]`);
  if (input) input.checked = true;
  syncCards();
}

function heroSvg(currentStage) {
  return `<svg viewBox="0 0 320 430"><rect width="320" height="430" fill="#151722"/><circle cx="160" cy="190" r="${90 + currentStage * 10}" fill="#f0b84f" opacity=".${currentStage + 1}"/><ellipse cx="160" cy="386" rx="104" ry="20" fill="#f0b84f" opacity=".18"/><path d="M110 365 C120 230 200 230 210 365Z" fill="${currentStage >= 4 ? "#f0b84f" : currentStage >= 2 ? "#4f8ef0" : "#6f7688"}"/><circle cx="160" cy="125" r="45" fill="#d6b274"/><path d="M116 116 C122 62 198 62 204 116 C190 92 132 92 116 116Z" fill="#2b1b13"/><circle cx="145" cy="126" r="4"/><circle cx="175" cy="126" r="4"/><path d="M146 146 C153 152 167 152 174 146" stroke="#3b2419" stroke-width="4" fill="none"/><text x="160" y="38" fill="#f5f2e8" text-anchor="middle" font-size="18" font-weight="800">${rank(currentStage)}</text><text x="160" y="62" fill="#f0b84f" text-anchor="middle" font-size="14" font-weight="700">Stade ${currentStage}</text></svg>`;
}
...
function startPulse() {
  ...
  pulseTimer = setInterval(() => {
    ...
    oscillator.frequency.value = beat++ % 4 ? 330 : 220;
...
el.create.onclick = () => {
  const id = selectedCoach();
  profile = {
    name: el.name.value.trim() || "Héros",
  ...
  pose = "welcome";
  log(`Aventure commencée avec ${coaches[id].name}.`);
  save();
  render();
  el.coachMsg.textContent = msg("start");
};

el.reset.onclick = () => {
  if (confirm("Réinitialiser tout le profil ?")) {
    stopPulse();
...
el.saveCoach.onclick = () => {
  const id = selectedCoach();
  profile.coach = id;
  pose = "welcome";
  log(`Nouveau coach choisi : ${coaches[id].name}.`);
  save();
  currentView = "home";
  render();
};
...
el.audioInput.onchange = (event) => {
  const file = event.target.files?.[0];
  if (file) {
    stopPulse();
    el.audio.src = URL.createObjectURL(file);
    el.audio.play();
    summaries();
  }
};
...
$$(".coach-choice-card").forEach((card) => {
  card.onclick = () => {
    card.querySelector('input[name="coach"]').checked = true;
    syncCards();
  };
});
