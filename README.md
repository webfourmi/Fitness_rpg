# Fitness RPG - V3

Web app locale de motivation sportive : quêtes quotidiennes, XP, niveaux, héros évolutif, et coachs.

## Fonctions

- Accueil avec résumé du héros et du coach.
- Choix de 6 coachs : Korvan, Xara, Violette, Elmin,Bazul et Satyne
- les coachs utilise 12 poses différentes selon la page ou la quête.
- Dashboard court avec résumé musique / quête.
- Sous-pages avec bouton Retour : Musique, Quêtes, Badges, Semaine.
- Phrases de motivation liées au coach et au type de quête.
- XP, niveaux, streak, badges, familiers et historique semaine.
- Sauvegarde locale via `localStorage`.



Dans GitHub : `Settings > Pages > Deploy from a branch >V5 clean.
